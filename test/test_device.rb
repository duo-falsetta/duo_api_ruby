...
class TestDevice < HTTPTestCase
  ...
  def setup_test_globals
    @device_api = DuoApi::Device.new(IKEY, SKEY, HOST)
  ...
  def test_create_device_cache
    ...
    required = { mkey: 'MKEY' }
    assert_nothing_raised{ @device_api.create_device_cache(**required) }
  end
  ...
  def test_create_device_cache_optional_args
    ...
    assert_nothing_raised{ @device_api.create_device_cache(**required, **optional) }
  end

  def test_create_device_cache_args_missing
    @mock_http.expects(:request).times(0)
    required = [:mkey]
    assert_raise_with_message(
      ArgumentError,
      missing_keywords_message(required)
    ){ @device_api.create_device_cache }
  end

  def test_add_device_cache_devices
    @mock_http.expects(:request).returns(@json_ok_hsh_resp)
    required = { mkey: 'MKEY', cache_key: 'CACHEKEY', devices: [{ device_id: 'DEVICEID1' }] }
    assert_nothing_raised{ @device_api.add_device_cache_devices(**required) }
  end

  def test_add_device_cache_devices_args_missing
    @mock_http.expects(:request).times(0)
    required = %i[mkey cache_key devices]
    assert_raise_with_message(
      ArgumentError,
      missing_keywords_message(required)
  ...
  def test_get_device_cache_devices
    ...
    required = { mkey: 'MKEY', cache_key: 'CACHEKEY' }
  ...
  def test_delete_device_cache_devices
    @mock_http.expects(:request).returns(@json_ok_hsh_resp)
    required = { mkey: 'MKEY', cache_key: 'CACHEKEY', devices: ['DEVICEID1'] }
